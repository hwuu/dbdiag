...
V2 架构变更：
- PhenomenonRetriever: 检索标准现象（推荐）
- StepRetriever: V1 步骤检索（deprecated）
"""
import sqlite3
import warnings
...
class StepRetriever:
    ...
    DEPRECATED: 请使用 PhenomenonRetriever 替代。
    """
    ...
    def __init__(self, db_path: str, embedding_service: EmbeddingService = None):
        ...
        """
        warnings.warn(
            "StepRetriever is deprecated. Use PhenomenonRetriever instead.",
            DeprecationWarning,
            stacklevel=2
        )
        self.db_path = db_path
